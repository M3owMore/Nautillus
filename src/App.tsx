...
const  App = () => {
  ...
  if(cookies.get('lang') == null){
    cookies.set('lang', 0, { path: '/', maxAge: 9999999999999})
  }
  ...
  const updateToken = async (getData:boolean) => {
    ...
    if(cookies.get('refresh') !== undefined && cookies.get('refresh') !== ""){
      ...
      .then((res:any) => {
        cookies.set('access', res.data.access, { path: '/',  maxAge: 180})
        if(getData){
          getUserInfo()
        }
  ...
  return (
    <BrowserRouter>
      ...
      <Routes>
        ...
        <Route path="/about" element={<About app={app}/>}/>

        <Route path="/blogs" element={<Blogs app={app}/>}/>
        <Route path="/blogs/:name" element={<Blog app={app}/>}/>
