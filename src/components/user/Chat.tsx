...
const ChatParent = (props:any) => {
    ...
    return (
        <>
            {chat ? <Chat chat={chat} user={props.user} app={props.app}/> : <></>}
        </>
    );
}
...
const ChatMessage = (props:any) => {
    return ( 
        <div className={'w-full flex flex-row items-end text-main ' + (props.msg.sender === props.app.user.name ? "justify-end" : "justify-start")}>
            {(props.msg.sender === props.user.name) ? <img src={props.user.img} className="w-8 rounded-full mr-4" alt="user"/> : <></>}
            <div className="mt-4">
        ...
        <div className='w-full h-full flex flex-col'>
            <div className='w-full h-full flex flex-col rounded-xl p-5'>
                ...
                <div className="relative w-full">
                    ...
                    <input autoComplete="false"  ref={inputRef} type="text" id="simple-search chat-input" className="bg-gray-900 border border-none text-main text-sm rounded-lg focus:border-none focus:outline-none block w-full pl-10 p-2.5 " placeholder="Send Message" required/>
                </div>
