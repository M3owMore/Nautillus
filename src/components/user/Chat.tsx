    ...
    useEffect(() => {
        if (lastJsonMessage !== null) {
            ...
            if (data['command'] === 'fetch_messages'){
                ...
                setMassages([...massages, data['message']])
            }
        }
    }, [lastJsonMessage]);
        ...
        <div className='w-full h-full flex flex-col'>
            <div className='w-full h-full flex flex-col rounded-xl p-5'>
                ...
                <div className='py-4 w-full h-full flex flex-col  overflow-y-scroll'>
                    {massages != null ?
                        massages.map((msg:object, j:number) => {
                            return <ChatMassage key={j} app={props.app} img={img} msg={msg}/>
                        })
                        : <></>
                    }
                </div>
                ...
                <div className="relative w-full">
                    <button onClick={handleMassageSend} className="absolute inset-y-0 left-0 flex items-center pl-3">
