...
const Box = (props:any) => {
    return (
        <button onClick={() => {window.location.href = `/courses/course/${props.course.title}`}} className="relative  hover:scale-105 hover:bg-bg-4 duration-200 flex flex-col h-full w-36 items-center justify-center bg-bg-3 rounded-xl p-1">
            <p className='text-secondary relative z-10 w-full text-center text-md mx-4 font-bold font-bpgESM'>{props.course.title}</p>
            <p className='text-main relative z-10 w-full text-center text-xs mx-4'>Level: {props.course.level}</p>
