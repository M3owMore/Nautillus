...
const  Header = (props:any) => {
    ...
    return (
      <>
        ...
        <div className={"backdrop-blur-sm bg-bg-1 bg-opacity-40 flex z-10 h-20 top-0 left-0 justify-between items-center w-screen px-4 py-2 text-main bg-bg transition-all" + ((scrollPosition > 200) ? " fixed header-appear" : "")} >
          
        {props.app.user.isUser  ? null
          :
          <div className="w-full h-full hidden tablet:flex justify-center items-center gap-1 font-alkSanet">
            <Link to="/"><p className={"mx-2 text-lg link link-underline  hover:text-secondary" + ((path === "/") ? " font-extrabold text-secondary hidden" : "")}>{props.app.langJson.words[props.app.lang].main}</p></Link>
            <Link to="/courses"><p className={"mx-2 text-lg link link-underline  hover:text-secondary" + ((path === "/courses") ? " font-extrabold text-secondary hidden" : "")}>{props.app.langJson.words[props.app.lang].courses}</p></Link>
            <Link to="/pricing"><p className={"mx-2 text-lg link link-underline  hover:text-secondary" + ((path === "/pricing") ? " font-extrabold text-secondary hidden" : "")}>{props.app.langJson.words[props.app.lang].pricing}</p></Link>
            <Link to="/contact"><p className={"mx-2 text-lg link link-underline  hover:text-secondary" + ((path === "/contact") ? " font-extrabold text-secondary hidden" : "")}>{props.app.langJson.words[props.app.lang].contact}</p></Link>
          </div>
        }
          ...
          <div className="w-full h-full flex justify-start tablet:justify-center items-center gap-1">
            <Link to="/">
              <div className="flex justify-center items-center hover:scale-110 duration-300">
                ...
                <p className="text-2xl m-1 font-bpgESM">Nautillus</p>
              </div>
            </Link>
          </div>
          ...
          <div className="w-full h-full flex  justify-end tablet:justify-center items-center gap-6">
            <div className="hidden tablet:flex items-center justify-center gap-6">
              {props.app.user.isUser ? null : <Dropdown options={options} onChange={(e:any) => {props.app.setLang(options.indexOf(e.value)); props.app.cookies.set('lang', options.indexOf(e.value), { path: '/' }); window.location.reload()}} placeholder={options[props.app.lang]} />}
              {props.app.user.isUser 
                ? 
                  <></>
                :
                  <Link to="/signin"><button className="bg-main text-black py-1 px-2 rounded-md font-bold">{props.app.langJson.words[props.app.lang].signIn}</button></Link>
              }
            </div>
            {props.app.user.isUser 
              ? 
        ...
        <div className={"backdrop-blur-md bg-bg-1 bg-opacity-40 fixed top-0 right-0 z-20 flex flex-col h-screen w-56  p-4 text-main transition-all" + (nav ? " translate-x-0" : " translate-x-full")}>
                      ...
                      <div className="flex flex-row justify-between">
                        ...
                        <div className="flex flex-col justify-between items-center w-full">
                          <div className="flex flex-row justify-around items-center w-full">
                            <p className='text-xs'>{props.app.user.name}</p>
              ...
              {props.app.user.isUser
                ...
                :
                  <div className="flex flex-row justify-evenly my-4 gap-2">
                    <Link to="/signin"><button className="bg-main text-xs text-black py-1.5 px-2 rounded-md font-bold hover:bg-zinc-300 duration-200">{props.app.langJson.words[props.app.lang].signIn}</button></Link>
                    <Link to="/signup"><button className="bg-main text-xs text-black py-1.5 px-2 rounded-md font-bold hover:bg-zinc-300 duration-200">{props.app.langJson.words[props.app.lang].signUp}</button></Link>
                  </div>
              }
