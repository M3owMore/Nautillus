...
const  Header = (props:any) => {
    ...
    return (
      <>
        ...
        <div className={"flex z-10 h-20 top-0 left-0 justify-between items-center w-screen px-4 py-2 text-main bg-bg transition-all" + ((scrollPosition > 200) ? " fixed header-appear backdrop-blur-sm bg-bg-1 bg-opacity-40" : "")} >
          ...
          <div className="w-full h-full flex justify-start tablet:justify-center items-center gap-1">
            <Link to="/">
              <div className="flex justify-center items-center hover:scale-110 duration-300">
                <img className="w-10 hover:w-14 duration-200 m-1" src={icon} alt="icon"/>
                <p className="text-2xl m-1 font-bpgESM">{"Nautillus".split("").map((char:string, i:number) => <span className="hover:text-4xl duration-200" key={i}>{char}</span>)}</p>
              </div>
            </Link>
          </div>
            ...
            <div className="hidden tablet:flex items-center justify-center gap-6">
              ...
              {props.app.user.isUser 
                ? 
                  <></>
                :
                  <Link to="/signin"><button className="bg-main hover:bg-zinc-300 duration-200 text-sm text-black py-1.5 px-3 rounded-md font-bold">{props.app.langJson.words[props.app.lang].signIn}</button></Link>
              }
            </div>
            {props.app.user.isUser 
              ? 
