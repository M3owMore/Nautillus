      ...
      ?
          <div className="relative flex flex-col p-6 rounded-2xl w-full max-w-xl" >
              ...
              <ul className="text-zinc-200 list-disc ml-4 m-1">
                  {course.topics.split("$").map((object:any, i:any) => (<li key={i} className="text-xs text-left ml-4 m-2">{object}</li>))}
              </ul>
              <br/>
              {course.big_description.split("\n").map((text:string, i:number) => <p key={i} className="text-main font-thin text-xs text-left m-1">{"ㅤ"}{text}</p>)}
              <p className="text-white text-lg text-center ">Level: <span className="text-secondary">{course.level}</span></p>
              
              <p className="text-zinc-400 text-sm text-center  mt-6"><span className="text-main font-bold text-3xl">10$</span>/month</p>
