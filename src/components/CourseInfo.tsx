...
const CourseInfo = (props:any) => {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            const res = await axios.get(props.app.backendURL + "" + courseName, {headers: {Authorization: 'Bearer ' + props.app.cookies.get('access')}});
  ...
  return (
    <div className="flex flex-col w-screen h-screen items-stretch">
        ...
        <div className="flex flex-col items-center justify-center h-screen gap-2">
            ...
            ?
              <>
                <h1 className="text-main text-5xl text-center">{course.name}</h1>
                ...
                <ul className="text-main list-disc ml-4">
                    {course.split("$").map((object:any, i:any) => (<li key={i} className="text-main text-md text-left ml-4 m-2">{object}</li>))}
                </ul>
