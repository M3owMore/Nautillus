...
const Course = (props:any) => {
    ...
    const [isCourse, setIsCourse] = useState(width <= 800)
    ...
    useEffect(() => {
        async function fetchLastPage() {
            setSpinner(true)
            await axios.get(props.app.backendURL + "api/user/courses/last/page/" + name, {headers: {Authorization: 'Bearer ' + props.app.cookies.get('access')}})
            .then((res) => {
    ...
    function returnLang(lang:any){
        return lang == 0 ? course.results[0].main_text : course.results[0].main_text_geo
    }
